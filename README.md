...
فایل dockerfile مجموعه ای از دستورات است که برای ایجاد یک ایمیج مورد استفاده قرار می‌گیرند. اجرای این فایل منجر به ایجاد یک image می‌شود. image یک الگوی آماده است که از روی فایل متنی dockerfile ساخته می‌شود. در حقیقت image شامل تمام اجزای مورد نیاز برای اجرای یک برنامه است مانند dependency ها و غیره. image فقط خواندنی است و نمی‌شود به طور مستقیم آن را تغییر داد. container یک نمونه یا instance اجرایی از image است. یعنی وقتی ما image را اجرا می‌کنیم یک container ایجاد می‌شود و برای خودش یک محیط ایزوله ایجاد می‌کند و تمام نیازمندی هایش را در محیط خودش ایجاد می‌کند.

## سوال ۲

کوبرنتیس یا kubernetes یک سیستم مدیریت و هماهنگی برای container ها است. کار اصلیش این است که وقتی تعداد زیادی container داریم کوبرنتیس مدیریت آنها را در دست می‌گیرد و کار های زیر را می‌کند:

### اجرای خودکار کانتینر ها
با مشخص کردن تعداد نمونه یا replica کوبرنتیس این تعداد از سرویس را اجرا می‌کند.

### مقیاس پذیری
اگر بار روی سیستم بالا برود کوبرنتیس خودش تعداد کانتینر ها را بالا می‌برد و اگر بار کم شود تعداد کانتینر ها هم کم می‌شود.

### load balancing
ترافیک کاربران را بین چند کانتینر به صورت متعادل توزیع می‌کند.

### self-healing
اگر یک کانتینر خراب شود و یا از کار بیفتد، کوبرنتیس دوباره آن را اجرا می‌کند.

### رابطه با داکر
داکر ابزار اصلی برای ساخت کانتینر ها است و کوبرنتیس ابزار مدیریت این کانتینر ها.
